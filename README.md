...
Built without PyTorch, MLX, or llama.cpp, and it ships as one binary.
...
Under greedy decoding, Ingot produces token-identical output to llama.cpp on every
quantization it supports. Its decode speed matches or beats llama.cpp.
...
Everything below works end to end as of September 2026.

| Area | State |
|---|---|
| GGUF loading (F32, F16, BF16, Q4_0, Q4_1, Q8_0, Q6_K) | Done |
| Llama 3 byte-level BPE tokenizer | Done, 42/42 cases identical to `llama-tokenize` |
| CPU reference forward pass | Done, greedy output identical to `llama-simple` |
| Metal backend: bandwidth-bound matvec, tiled prefill, flash attention | Done, greedy output identical to `llama-simple` on Q4_0, Q8_0, F16 |
...
| Prefill speed vs llama.cpp (M5) | Partial, roughly 75 to 80 percent |
| K-quants beyond Q6_K | Planned |
...
- Xcode Command Line Tools. Shaders compile at runtime, so the Command Line Tools are
  enough on their own.
...
| Endpoint | Purpose |
|---|---|
| `POST /v1/chat/completions` | Chat completions, with or without SSE streaming |
...
Prefill pushes the whole prompt through `forward_batch` in chunks. Decode then calls
`forward` once per generated token. The glue in `engine.cpp` is the only place that knows
about both, so the CLI and the server share identical prefill, sampling, and streaming
behaviour.
...
1. Embedding on the CPU. Token embeddings are single rows, so they are dequantized
   straight into the shared `x` buffer, and there is no gather kernel.
2. One command buffer and one encoder for all 28 layers. Each layer is the graph below.
...
Decode is memory-bound. Every weight byte is read exactly once per token, so the matvec
kernels are built to saturate bandwidth:
...
- Prefill computes `S = QKᵀ` and `O += PV` as 8×8 tile products on the SIMD-group matrix
  units, one SIMD group per (head, 8 queries), rescaling `O` with a diagonal matrix when
  the running max grows.

Everything else (RMSNorm, RoPE, SwiGLU, f32 to f16 conversion) is a small elementwise or
...
Measured on an M5 with 24 GB, back to back with `tests/bench.sh`, in tokens per second.
...
multi-tile flash attention as well as the first tile.
...
- Llama architecture only, tested on Llama 3.2 3B Instruct. Other GGUF models with the
  same layout should load; other architectures fail at load time.
- Quantization types: F32, F16, BF16, Q4_0, Q4_1, Q8_0, Q6_K. Other K-quants are
  unsupported.
