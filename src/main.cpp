...
static void usage() {
    fprintf(stderr,
            "usage:\n"
            "  llm info <model.gguf>                          print model metadata and tensor table\n"
            "  llm tokenize <model.gguf> [opts] [-p TEXT]     print token ids as [a, b, c]\n"
            ...
            "  llm detokenize <model.gguf> ID...              print the text for token ids\n"
            "  llm bench <model.gguf> [-p 128] [-n 32] [-r 3] [--backend B]   measure prompt and generation tok/s\n"
            "  llm kbench <model.gguf> [-n 128] [-i 20] [-t blk.0.ffn_up.weight]   time one matmul kernel\n"
            "  llm run <model.gguf> -p TEXT [opts]            generate text\n"
            ...
            "  llm chat <model.gguf> [opts]                   interactive chat (/reset, /quit)\n"
            "  llm serve <model.gguf> [--port 8080] [--host 127.0.0.1] [--ctx 8192]   OpenAI-compatible API\n"
